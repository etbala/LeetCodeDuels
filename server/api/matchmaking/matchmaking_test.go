...
func TestMatchmaking(t *testing.T) {
	...
	player1 := &models.Player{ID: "Player1", Tags: []string{"Tag1"}, Matched: make(chan *models.Lobby, 1), JoinedAt: time.Now(), ForceMatch: false}
	player2 := &models.Player{ID: "Player2", Tags: []string{"Tag1"}, Matched: make(chan *models.Lobby, 1), JoinedAt: time.Now(), ForceMatch: false}
...
func TestMatchmakingWithOverlappingFlags(t *testing.T) {
	...
	player1 := &models.Player{ID: "Player1", Tags: []string{"Tag1", "Tag2"}, Matched: make(chan *models.Lobby, 1), JoinedAt: time.Now(), ForceMatch: false}
	player2 := &models.Player{ID: "Player2", Tags: []string{"Tag2", "Tag3"}, Matched: make(chan *models.Lobby, 1), JoinedAt: time.Now(), ForceMatch: false}
...
func TestMatchmakingWithNoCommonFlagsAndTimeout(t *testing.T) {
	...
	player1 := &models.Player{ID: "Player1", Tags: []string{"Tag4"}, Matched: make(chan *models.Lobby, 1), JoinedAt: time.Now(), ForceMatch: true}
	player2 := &models.Player{ID: "Player2", Tags: []string{"Tag5"}, Matched: make(chan *models.Lobby, 1), JoinedAt: time.Now().Add(1 * time.Second), ForceMatch: true}
...
func TestMatchmakingPrioritizesOldestPlayer(t *testing.T) {
	...
	oldestPlayer := &models.Player{ID: "OldestPlayer", Tags: []string{"Tag1"}, Matched: make(chan *models.Lobby, 1), JoinedAt: time.Now()}
	time.Sleep(1 * time.Second) // Ensure there's a noticeable difference in JoinedAt times

	newerPlayer := &models.Player{ID: "NewerPlayer", Tags: []string{"Tag1"}, Matched: make(chan *models.Lobby, 1), JoinedAt: time.Now()}
...
func validateMatch(t *testing.T, player *models.Player, expectedMatchID string) {
	select {
